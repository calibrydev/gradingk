<?php
    ...
    if(isset($_POST['register'])){
        ...
        foreach($_POST['unit'] as $adm){
            ...
            $sql="INSERT INTO `enrolled`(`adm`,`unitname`,`status`,`teacher`,`hours`) VALUES (:adm,:unitname,:status,:teacher,:hours)";
            $stmt=$conns->prepare($sql);
            $stmt->execute(['adm'=>$adm,'unitname'=>$unit,'status'=>$status,'teacher'=>$teacher,'hours'=>2]); 
